...
const io = new Server(server, {
  ...
  /* socket.on('placeBid', async (data) => {
    try {
      // data might have { auctionId, playerId, teamId }
      // you can also store the user’s role/team if you want to block them from bidding for other teams
      const { auctionId, playerId, teamId, increment } = data;

      const updatedPlayer = await placeBidLogic({ auctionId, playerId, teamId, increment });
      bidHistory.push(updatedPlayer.final_bid);
