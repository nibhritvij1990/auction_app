...
router.delete('/:playerId', authMiddleware, async (req, res) => {
  ...
  router.put('/bulk-update', async (req, res) => {
    // body is an array of { _id, fieldsToChange }, for example
    try {
      const updates = req.body; // e.g. [ { _id: "...", base_price: 100, category: "Bowler" }, ... ]
      let updatedCount = 0;
  
      // We can use a loop or a bulkWrite approach
      // 1) Example using bulkWrite:
      const ops = updates.map(u => ({
        updateOne: {
          filter: { _id: u._id },
          update: { $set: { ...u } } // remove _id from the set though
        }
      }));
